...
const CategoriesPage = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const message = err?.response?.data?.message || "Category save failed.";
      setError(message);
      toast.error(message);
    } finally {
      setSubmitting(false);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const message = err?.response?.data?.message || "Category delete failed.";
      setError(message);
      toast.error(message);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#4A1942] p-6 md:p-8 space-y-6">
      ...
      <SectionHeader
        title="Categories"
        subtitle="Manage your achar product categories"
        action={
          <button
      ...
      {loading ? (
        <GlassCard className="p-6 text-purple-200">Loading categories...</GlassCard>
      ) : filtered.length === 0 ? (
        <GlassCard className="p-6 text-purple-200">No categories found.</GlassCard>
      ) : view === "grid" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((cat) => (
            <div key={cat._id} className="group bg-white/5 backdrop-blur-sm rounded-3xl p-6 border border-white/10 hover:border-orange-300/30 hover:bg-white/10 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm text-purple-300 mb-4">/{cat.slug}</p>
                <div className="flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-1.5 text-purple-200">
                    <Package size={16} className="text-orange-300" />
                    <span className="font-semibold text-white">{cat.products || 0}</span> Products
                  </div>
                  <div className="flex items-center gap-1.5 text-purple-200">
                    <FolderTree size={16} className="text-green-300" />
                    <span className="font-semibold text-white">{cat.subCategories || 0}</span> Sub
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between">
                  <StatusBadge status={cat.status} text={cat.status === "active" ? "Active" : "Inactive"} />
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((cat) => (
                <tr key={cat._id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-sm text-purple-300">/{cat.slug}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-white">{cat.products || 0}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-white">{cat.subCategories || 0}</td>
                  <td className="px-6 py-4">
                    <StatusBadge status={cat.status} text={cat.status === "active" ? "Active" : "Inactive"} />
                  </td>
